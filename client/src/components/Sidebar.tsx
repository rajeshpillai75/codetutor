  ...
  return (
    <aside className="bg-white w-64 flex-shrink-0 flex flex-col h-full overflow-y-auto hidden md:block shadow-md">
      ...
      <nav className="flex-1 py-4">
        ...
        <div 
          className={`flex items-center gap-2 px-4 py-2 ${location === '/demo/code-execution' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-100'} rounded transition-colors mt-1 cursor-pointer`}
          onClick={() => navigateTo('/demo/code-execution')}
        >
          <i className="ri-computer-line text-blue-500"></i>
          <span>Code Execution Demo</span>
          <span className="ml-auto text-xs px-1.5 py-0.5 rounded-full bg-green-100 text-green-600 font-medium">New</span>
        </div>
      </nav>
